...
inline bool load_chain_from_urdf(const char* urdf_xml,
  ...
  UrdfLinkDesc links_desc[ARM_LIB_URDF_MAX_JOINTS + 1U];
  uint16_t joint_count = 0U;
  uint16_t link_count = 0U;
  if (!parse_urdf_joints(urdf_xml, joints, ARM_LIB_URDF_MAX_JOINTS,
                         &joint_count)) {
    return false;
  }
  (void)parse_urdf_links(urdf_xml, links_desc, ARM_LIB_URDF_MAX_JOINTS + 1U,
                         &link_count);

  uint16_t path_indices[N];
  uint16_t path_length = 0U;
  if (!find_urdf_joint_path(joints, joint_count, root_link, tip_link, path_indices,
                            N, &path_length)) {
    return false;
  }
  ...
  for (uint16_t i = 0; i < N; ++i) {
    const UrdfLinkDesc* child_link_desc =
        find_urdf_link_by_name(links_desc, link_count,
                               joints[path_indices[i]].child_link);
    links[i] = make_link_from_urdf_joint(joints[path_indices[i]], child_link_desc);
  }
  *chain_out = make_serial_chain(links);
  return true;
}
...
inline bool load_movable_chain_from_urdf(const char* urdf_xml,
  ...
  if (count_movable_urdf_joints_on_path(joints, path_indices, path_length) !=
  ...
  for (uint16_t i = 0; i < path_length; ++i) {
    const UrdfJointDesc& joint = joints[path_indices[i]];
    ...
    if (movable_index == 0U) {
      ...
      links[movable_index] = make_link_from_urdf_joint(
          joint, find_urdf_link_by_name(links_desc, link_count, joint.child_link));
    } else {
      links[movable_index] = make_link_from_urdf_joint(
          joint, find_urdf_link_by_name(links_desc, link_count, joint.child_link));
  ...
  *chain_out =
      make_serial_chain(links, base_frame, ToolFrame(accumulated_fixed));
  return true;
}
