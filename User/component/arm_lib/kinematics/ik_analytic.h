...
inline IKResult<N> solve_ik_analytic(
  ...
  for (uint16_t i = 0; i < candidates.count; ++i) {
    ...
    const Scalar reference_cost =
        has_reference ? solver::configuration_distance(chain, q_candidate,
                                                       reference)
                      : q_candidate.norm();
    const Scalar cost =
        within_limits
            ? (reference_weight * reference_cost)
            : (1000.0f * limit_violation + reference_weight * reference_cost);
