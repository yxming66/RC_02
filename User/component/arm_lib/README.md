...
`arm_lib` is the current serial-manipulator algorithm library used by this
project. It has replaced the old `armk_*` C stack and the temporary
`arm6dof` compatibility route in the main firmware path.
...
Those paths all use the current C++ `arm_lib` implementation.
...
- `adapter/`
  - adapter layer between `arm_lib` and the existing toolbox math library
- `model/`
...
- `demo/`
  - self-check style demos for 3R and URDF-based chains
