...
constexpr float kMotorCurrentResolution = 16384.0f;
...
int16_t ResolveCurrentLsb(MOTOR_RM_Module_t module) {
    switch (module) {
        case MOTOR_M2006:
            return 10000;
        case MOTOR_M3508:
...
bool MotorProtocol<MotorKind::RM, Model>::TryGetRotorFeedback(float& rotor_position_rad,
    ...
    rotor_velocity_rad_s = static_cast<float>(raw->raw_speed);
    if (param_.reverse) {
        rotor_velocity_rad_s = -rotor_velocity_rad_s;
    }

    const float current_lsb = static_cast<float>(ResolveCurrentLsb(param_.module));
    torque_current = (current_lsb > 0.0f)
        ? (static_cast<float>(raw->raw_current) * current_lsb / kMotorCurrentResolution)
        : 0.0f;
    if (param_.reverse) {
