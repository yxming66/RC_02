...
inline Matrixf<N, 6> damped_task_pseudoinverse(
    const Jacobian6xN<N>& jacobian, Scalar damping) {
  Matrixf<N, 6> out = matrixf::zeros<N, 6>();
  (void)damped_task_pseudoinverse_safe(jacobian, damping, &out);
  return out;
}
...
inline bool damped_task_pseudoinverse_safe(
  ...
  Matrixf<6, 6> task_inverse = matrixf::zeros<6, 6>();
  if (!math::solve_symmetric_ldlt(task_matrix, task_identity, &task_inverse)) {
