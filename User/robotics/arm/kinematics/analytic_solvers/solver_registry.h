...
template <>
inline AnalyticIKRegistry<6, 6> build_registry<6, 6>(
    AnalyticSolverPreset preset) {
  AnalyticIKRegistry<6, 6> registry;
  ...
  if (preset == AnalyticSolverPreset::kAuto ||
      preset == AnalyticSolverPreset::kPieper6R) {
    (void)register_analytic_plugin(analytic::make_pieper_6r_plugin(),
                                   &registry);
  }
