...
inline Scalar score_ik_solution(const SerialChain<N>& chain,
                                ...
                                bool from_analytic,
                                bool from_numeric) {
  const bool has_primary_reference =
      request.use_current || request.use_reference;
  const JointVec<N> primary_reference =
      request.use_current
          ? request.current
          : (request.use_reference ? request.reference
                                   : chain.zero_configuration());
  IkCandidateScoreWeights weights = make_ik_candidate_score_weights(
      1.0f, options.singularity_threshold);
  const IkCandidateScore candidate_score = score_ik_candidate(
      chain, q, primary_reference, has_primary_reference,
      chain.joint_limits(), weights);
  Scalar score = candidate_score.total;
  if (request.use_current && request.use_reference) {
...
template <int N>
inline bool numeric_ik_solution_step_within_options(
    const SerialChain<N>& chain,
    const IKRequest<N>& request,
    const IKOptions& options,
    const JointVec<N>& q,
    Scalar* max_delta_out = nullptr) {
  if (!options.enable_solution_step_limit ||
      options.max_solution_joint_step <= ARM_LIB_EPSILON) {
    if (max_delta_out != nullptr) {
      *max_delta_out = 0.0f;
    }
    return true;
  }

  JointVec<N> reference = toolbox_adapter::zero_joint_vec<N>();
  bool has_reference = false;
  if (request.use_reference) {
    reference = request.reference;
    has_reference = true;
  } else if (request.use_current) {
    reference = request.current;
    has_reference = true;
  } else if (request.use_seed) {
    reference = request.seed;
    has_reference = true;
  }
  if (!has_reference) {
    if (max_delta_out != nullptr) {
  ...
  const Scalar max_delta = max_ik_joint_delta(chain, reference, q);
  if (max_delta_out != nullptr) {
    *max_delta_out = max_delta;
  }
  return max_delta <= options.max_solution_joint_step + ARM_LIB_EPSILON;
}
...
inline IKResult<N> solve_ik_numeric_once(const SerialChain<N>& chain,
  ...
  for (uint16_t iter = 0; iter < options.max_iterations; ++iter) {
    ...
    if (state.weighted_error_norm <= options.error_tolerance) {
      Scalar max_solution_step = 0.0f;
      if (!numeric_ik_solution_step_within_options(
              chain, request, options, q, &max_solution_step)) {
        result.status = IkStatus::kNoConvergence;
        result.q = q;
        result.stats.final_error_norm = state.weighted_error_norm;
        result.stats.final_step_norm = max_solution_step;
        result.diagnostics.stage = IkFailStage::kPostCheck;
        result.diagnostics.reason = IkFailureReason::kNoConvergence;
        result.diagnostics.final_error_norm = state.weighted_error_norm;
        result.diagnostics.final_step_norm = max_solution_step;
    ...
    const auto make_trial_candidate =
        [&](const JointVec<N>& step, Scalar scale) {
          JointVec<N> candidate = q + step * scale;
          candidate = solver::align_revolute_to_reference(chain, candidate, q);
          if (request.use_reference) {
            candidate = solver::align_revolute_to_reference(
                chain, candidate, request.reference);
          }
          if (options.clamp_to_limits_each_step) {
            candidate = solver::clamp_to_chain_limits(chain, candidate);
          }
          return candidate;
        };

    const JointVec<N> full_step = dq + dq_secondary;
    JointVec<N> q_candidate = make_trial_candidate(full_step, 1.0f);
    ...
    if (options.method == IkMethod::kLevenbergMarquardt) {
      ...
      if (candidate_error > (state.weighted_error_norm + acceptance_slack) &&
          used_secondary) {
        JointVec<N> primary_only_candidate =
            make_trial_candidate(dq, 1.0f);
      ...
      if (accepted_error > (state.weighted_error_norm + acceptance_slack)) {
        const Scalar scales[3] = {0.5f, 0.25f, 0.125f};
        for (uint16_t i = 0; i < 3U; ++i) {
          const JointVec<N> trial =
              make_trial_candidate(full_step, scales[i]);
          const Scalar trial_error =
              evaluate_numeric_ik_error(chain, request.target, trial, options);
          if (trial_error < accepted_error) {
            accepted_candidate = trial;
            accepted_error = trial_error;
          }
          if (trial_error <=
              (state.weighted_error_norm + acceptance_slack)) {
            break;
          }
        }
      }
