...
bool parse_urdf_joints(const char* urdf_xml,
  ...
  return parse_urdf_joints(urdf_xml, joints, max_joint_count, joint_count_out,
                           &workspace);
}
...
bool parse_urdf_joints(const char* urdf_xml,
  ...
  if (urdf_xml == nullptr || joints == nullptr || joint_count_out == nullptr ||
      max_joint_count == 0U || workspace == nullptr) {
    return false;
  }
  ...
  while (count < max_joint_count) {
    ...
    char* block = workspace->joint_block;
    if (block_len >= sizeof(workspace->joint_block)) {
      return false;
    }
...
bool parse_urdf_links(const char* urdf_xml,
  ...
  UrdfParserWorkspace workspace;
  return parse_urdf_links(urdf_xml, links, max_link_count, link_count_out,
                          &workspace);
}
...
bool parse_urdf_links(const char* urdf_xml,
  ...
  if (urdf_xml == nullptr || links == nullptr || link_count_out == nullptr ||
      max_link_count == 0U || workspace == nullptr) {
    return false;
  }
  ...
  while (count < max_link_count) {
    ...
    char* block = workspace->link_block;
    if (block_len >= sizeof(workspace->link_block)) {
      return false;
    }
      ...
      if (inertial_end != nullptr) {
        const size_t inertial_len =
            static_cast<size_t>(inertial_end + strlen("</inertial>") -
                                inertial_tag);
        char* inertial_block = workspace->inertial_block;
        if (inertial_len >= sizeof(workspace->inertial_block)) {
          return false;
        }
...
bool find_urdf_joint_path(const UrdfJointDesc* joints,
  ...
  UrdfParserWorkspace workspace;
  return find_urdf_joint_path(joints, joint_count, root_link, tip_link,
                              path_indices, max_path_length, path_length_out,
                              &workspace);
}
...
bool find_urdf_joint_path(const UrdfJointDesc* joints,
  ...
  if (joints == nullptr || root_link == nullptr || tip_link == nullptr ||
      ...
      workspace == nullptr || joint_count > ARM_LIB_URDF_MAX_JOINTS) {
    return false;
  }

  memset(workspace->used_joints, 0, sizeof(workspace->used_joints));
  uint16_t depth = 0U;
  if (!find_path_dfs(joints, joint_count, root_link, tip_link, path_indices,
                     max_path_length, 0U, workspace->used_joints, &depth)) {
    return false;
  }
