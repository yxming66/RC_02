...
class CartesianServo {
 ...
 private:
  ...
  Transform resolve_target_pose(const Transform& current_pose,
    ...
    const frames::CartesianCommandFrame frame =
        (target.frame == CartesianTargetFrame::kToolLocal)
            ? frames::CartesianCommandFrame::kTool
            : frames::CartesianCommandFrame::kBase;
    return frames::resolve_delta_tcp_pose(current_pose, target.pose, frame);
  }
