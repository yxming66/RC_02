...
typedef struct {
  ...
  float mid_move_distance_m;  /* 第一段固定快速靠近/中段移动距离，单位 m；<=0 使用 mid_move_ms。 */
  ...
  float rear_retract_move_distance_m; /* 第二段固定快速靠近/后侧动作移动距离，单位 m；<=0 使用 rear_retract_move_ms。 */
  ...
  float final_move_distance_m; /* 收尾/全收杆后离开距离，单位 m；<=0 使用 final_move_ms。 */
