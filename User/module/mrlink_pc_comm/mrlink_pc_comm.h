...
typedef enum {
    ...
    PC_FEEDBACK_RETRY_REGION_2 = 0x08, /* STM32->PC 一次性二区重试请求命令 */
...
/* 请求向 PC 发送一次二区重试命令。 */
bool MrlinkPc_RequestRetryRegion2(void);

/* 返回是否有待发送的二区重试命令。 */
bool MrlinkPc_HasRetryRegion2Request(void);

/* 清除待发送的二区重试命令，通常在发送成功后调用。 */
void MrlinkPc_ClearRetryRegion2Request(void);

/* 构造二区重试命令 mrlink 帧；无待发送命令或缓存不足时返回 0。 */
uint16_t MrlinkPc_BuildRetryRegion2Frame(uint8_t *tx_buf,
                                        uint16_t buf_size);
