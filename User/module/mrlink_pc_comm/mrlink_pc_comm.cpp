...
static wire::RetryRegion2Cmd s_retry_region_2_cmd{};
static bool s_retry_region_2_pending = false;
...
extern "C" bool MrlinkPc_CommInit(void) {
  ...
  s_retry_region_2_pending = false;
  s_retry_region_2_cmd.retry = 0u;
...
extern "C" bool MrlinkPc_RequestRetryRegion2(void) {
  s_retry_region_2_cmd.retry = 1u;
  s_retry_region_2_pending = true;
  return true;
}

extern "C" bool MrlinkPc_HasRetryRegion2Request(void) {
  return s_retry_region_2_pending;
}

extern "C" void MrlinkPc_ClearRetryRegion2Request(void) {
  s_retry_region_2_pending = false;
  s_retry_region_2_cmd.retry = 0u;
}

extern "C" uint16_t MrlinkPc_BuildRetryRegion2Frame(uint8_t *tx_buf,
                                                      uint16_t buf_size) {
  if (!s_retry_region_2_pending || tx_buf == nullptr) {
    return 0u;
  }

  return s_bus.Publish(wire::kFeedbackRetryRegion2, s_retry_region_2_cmd,
                       tx_buf, buf_size);
}
