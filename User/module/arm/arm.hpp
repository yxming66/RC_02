...
extern volatile uint8_t g_arm_gravity_only_torque_enable;
...
struct RuntimeDebugData {
  ...
  float motor_command_position[ARM_JOINT_COUNT];
  float motor_command_velocity[ARM_JOINT_COUNT];
  float motor_command_torque[ARM_JOINT_COUNT];
  float motor_command_kp[ARM_JOINT_COUNT];
  float motor_command_kd[ARM_JOINT_COUNT];
  bool joint_online[ARM_JOINT_COUNT];
  bool joint_pending[ARM_JOINT_COUNT];
  mr::motor::MotorState motor_state[ARM_JOINT_COUNT];
...
class Runtime {
 public:
  ...
  mr::motor::DmJ4310Motor* joint3_motor() { return joint3_motor_; }
 ...
 private:
  ...
  bool RunGravityOnlyTorqueCycle();
  bool RunZeroTorqueCycle();
  ...
  mr::motor::DmJ4310Motor* joint3_motor_ = nullptr;
  ...
  bool gravity_only_torque_was_enabled_ = false;
