...
- UART8 红外命令 `A5 03` 只建立三层放矿锁，不自动启动 legacy 动作。PC 在 `0x9A.zone3_action_locked=1` 后使用 V3 提交唯一的动作 38。动作 38 持有全部底盘和上层资源，准备到位后无限等待新的命令 4，忽略目标格光电检测；完成释放后仍保持 RUNNING 和底盘锁，命令 5 才结束 Job。命令 6 触发安全回撤并返回 `CANCELLED`，之后仍需命令 5 解除红外锁。
