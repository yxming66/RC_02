...
其它 `state` 值按未准备完毕处理，即执行全灭灯效。取矛头等待对接动作会把 `state=1` 作为新协议的对接完成条件；UART8 红外命令 `02` 仍兼容。只有矿仓平台已到 `SPEARHEAD_DOCK_WAIT`、取矛头舵机当前姿态为 `DOCK_WAIT` 且到位后，固件才记录本机 ready 时间；对接完成帧的接收时间必须不早于该 ready 时间。提前发送的旧 `state=1`/`02` 不会在机构随后到位时补触发，发送端必须在机构就绪后重新发送。等待对接信号没有超时：未收到有效完成信号且未收到 CANCEL/ABORT 时，动作保持 RUNNING 并持续保持平台、舵机和夹爪命令。
...
| 15 | ROD_DOCK_WAIT；机构到位后无限等待有效对接完成信号，仅 CANCEL/ABORT 可中止 |
