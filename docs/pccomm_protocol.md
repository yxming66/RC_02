...
| 38 | RELEASE_IR_LIFT_DETECT，三层放矿单事务：启动后独占 CHASSIS、POLE、ARM、STORE、ROD 和共享气路；命令 3 只执行一次放矿并回等待位，Job 继续 `RUNNING` 且不释放资源；仅命令 4 正常结束，或 PC V3 `CANCEL` 中止结束 |

PC 必须通过 V3 `SUBMIT(action=38)` 创建唯一 Job，启动成功后 RC02 自动建锁并令 `0x9A.zone3_action_locked=1`。动作 38 在整段事务中持续占有 CHASSIS、POLE、ARM、STORE 和共享阀资源，并持续输出底盘零速度。准备顺序为 Pole 展开、从矿仓上膛（如需要）、机械臂到 `WAIT_RELEASE_ORE`；平台、机械臂和 Pole 到位后才记录许可就绪时间。早于就绪时间的命令 `3` 不会补触发，R1 必须在就绪后重新发送。动作 38 完全忽略前方目标格光电/SICK 有无矿结果，没有 step1/step2；未收到有效命令 `3` 时无限保持且不释放。收到命令 `3` 后执行完整放矿，完成后回到 `WAIT_RELEASE_ORE` 并继续保持 Job 和底盘锁。命令 `4` 才让 Arm 回到 `STANDBY`，同时解锁，并在到位后返回 `SUCCEEDED`；需要中止时使用 V3 `CANCEL`，返回 `CANCELLED`。
