...
一键放矿流程还会直接发送灯效：目标为低层转换仓时发送 `0x04`，目标为高层转换仓时发送 `0x05`；三层抬升检测放矿进入等待被抬升阶段时发送 `0x08`。`PC_FEEDBACK_AUTO_ACTION` 会按真实结果上报成功、失败或中止；灯效同步发送成功灯效 `0x07` 或失败灯效 `0x03`。
...
2. `busy=0 && finished=1 && result=0` 表示最近一次一键动作成功结束。
3. `busy=0 && finished=1 && result=1` 表示失败，必须读取 `failure_mask`。
4. `ABORT` 上报为 `finished=1, result=1`，并设置 `failure_mask.ABORTED`。
...
分段位定义与 `0x96` 相同。STM32 仅在对应分支真实成功事件发生时设置完成位；失败和中止不会把未完成位补成完成。终态结果保持到新的 START 覆盖或 PC 发送 `ACK(job_id)`。
