...
3. `RELEASE_STEP1`、`RELEASE_LIFT_DETECT_STEP1`、`RELEASE_IR_LIFT_DETECT_STEP1`、`ROD_SPEARHEAD_STEP1/STEP2`、`ROD_DOCK_WAIT` 上报真实结果；其 `result=1` 时必须读取 `failure_mask`。
...
固定包头为 `0x4D`，`command` 为一个二进制数据字节，不使用 CRC：

| command | 含义 | 固件行为 |
|---:|---|---|
| `1` | 在线 | 刷新红外在线状态 |
| `2` | 对接完成 | 刷新对接完成状态及时间戳 |
| `3` | 允许放矿 | 刷新放矿允许状态，供三层放矿动作使用 |
| `4` | 中止放矿 | 立即撤销放矿允许，并递增中止事件计数 |
| `5` | 允许移动 | 锁存 R2 允许移动状态 |

所有合法命令都会刷新红外在线状态；`command` 不在 `1~5` 时按无效帧统计。对接完成和允许放矿带新鲜期，允许移动在本次上电期间锁存。payload 为 28 字节，Python unpack 格式为 `<12B4I`。
...
| 3 | u8 | `r2_leave_zone1_allowed` | 是否收到过命令 `5` 允许移动，`0/1` |
| 4 | u8 | `claw_open` | 命令 `3` 的允许放矿状态是否仍在新鲜期；命令 `4` 会立即清零 |
| 5 | u8 | `cleared_ore_id` | 简化协议未携带，固定反馈 `0xFF` |
| 6 | u8 | `zone3_r2_state` | 简化协议未携带，固定反馈 `0` |
| 7 | u8 | `last_dock_complete_cmd` | 是否收到过命令 `2`，`0/1` |
| 8 | u8 | `last_r2_leave_zone1_cmd` | 是否收到过命令 `5`，`0/1` |
| 9 | u8 | `last_claw_open_cmd` | 最近放矿命令，`0=等待`、`1=允许`、`2=中止` |
| 10 | u8 | `last_command` | 最近成功解析的原始命令，取值 `1~5` |
| 11 | u8 | `release_abort_count_lsb` | 命令 `4` 累计接收次数低 8 位，数值变化表示新的中止事件 |
...
动作 `38/39/40` 使用 UART8 的命令 `3/4`。目标格有矿时不会失败，也不会因命令 `3` 强制放矿，而是持续保持；矿移走并连续空闲 500 ms 后，已收到的允许信号可继续流程。运行任意阶段收到命令 `4` 后，固件按 `RELEASE_ORE -> RELEASE_ORE_ASSIST -> WAIT_RELEASE_ORE -> VERTICAL` 的逆序安全路径回撤；已经释放吸盘的阶段保持吸盘关闭，未释放阶段保持吸附。到达 `VERTICAL` 后动作结束并通过 `PC_FEEDBACK_AUTO_ACTION` 返回 `ABORTED`。

### 6.3 红外对接

UART8 简化帧为两个字节：`0x4D command`。命令定义为 `1=在线`、`2=对接完成`、`3=允许放矿`、`4=中止放矿`、`5=允许移动`，不使用 CRC 或 ACK。
