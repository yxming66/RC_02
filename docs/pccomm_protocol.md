...
一键放矿流程还会直接发送灯效：目标为低层转换仓时发送 `0x04`，目标为高层转换仓时发送 `0x05`；三层抬升检测放矿进入等待被抬升阶段时发送 `0x08`。`PC_FEEDBACK_AUTO_ACTION` 当前对 PC 只上报动作结束成功；内部灯效仍可根据本机状态发送成功灯效 `0x07` 或失败灯效 `0x03`。
...
| 3 | u8 | `result` | 仅 `finished=1` 有效；当前固件对 PC 固定上报 `0=SUCCESS` |
| 4 | u16 | `failure_mask` | 当前固件对 PC 固定上报 0，内部失败不通过该字段暴露 |
...
2. `busy=0 && finished=1 && result=0` 表示最近一次一键动作已经结束；当前固件不区分内部成功/失败，统一按成功结束上报给 PC。
3. `result=1` 和非 0 `failure_mask` 为旧协议保留；当前固件不会主动通过 `PC_FEEDBACK_AUTO_ACTION` 上报失败。
4. `ABORT` 也只作为动作结束上报：`finished=1`，`result=0`，`failure_mask=0`。
