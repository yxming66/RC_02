...
PC 控制命令实际生效还要求遥控器进入 PC 控制页：当前代码条件为 `sw_l == UP && sw_r == UP` 且 PC 链路在线。上位机可通过 `PC_FEEDBACK_STATUS.command_source == 1` 判断当前是否由 PC 接管。PC 成功分发自动动作后会建立一个受限接管状态：只要 PC 链路仍在线，动作结束会恢复 PC 命令路由，避免 Pole 在 AutoOre 终态交接时进入 RELAX；PC 下发 ABORT、PC 链路离线，或操作手从 `UP-UP` PC 页明确拨离，都会清除该接管状态。
...
- `busy=1` 期间固件保持自动命令路由和该动作的资源租约。PC 可以继续刷新心跳和连续控制帧，但不得假设这些帧已接管被占用模块；只有真实 `SUCCESS/FAIL/ABORTED` 终态后才释放租约。唯一例外是 `action=3`：动作运行期间 `PC_CMD_CHASSIS (0x10)` 可实时控制底盘，Pole、Arm 和矿仓仍由 AutoOre 控制。
...
固件每 20 ms 尝试发送以下反馈批次：`0x81, 0x96, 0x90, 0x91, 0x93, 0x94, 0x95, 0x98, 0x92, 0x9B, 0x9C, 0x9D, 0x9A, 0xA0`。`0x02` 开始比赛/一区重试命令可由遥控器左右拨杆从已知非 PC 页切入 PC 页（`sw_l=UP && sw_r=UP`）的变化沿触发，也可由 MineInput 射频 `mode=1` 触发；上电时若拨杆已在 UP-UP，不会因初始状态未知而误发。`0x08` 二区重试命令可由 MineInput 射频 `mode=2` 或 PC 页左摇杆 X 越过重试阈值的变化沿触发。`0x17` 仅作为 PC 下发的 camera yaw 命令使用，固件不再把它作为命令镜像回传。`0x97/0x99` 旧红外矿种反馈在首次收到或内容变化后持续低频追加，当前周期约 200 ms；`0x9A` 为新 R1/R2 红外对接协议状态反馈，随 50 Hz 常规反馈发送。

| cmd | 名称 | payload | Python unpack | 说明 |
|---:|---|---:|---|---|
| `0x02` | `PC_FEEDBACK_START_MATCH` | 1 | `<B` | 开始比赛/一区重试命令；`start=1` |
| `0x08` | `PC_FEEDBACK_RETRY_REGION_2` | 1 | `<B` | 二区重试请求；`retry=1` |
...
### 5.0 一次性控制反馈 `0x02/0x08`

`PC_FEEDBACK_START_MATCH (0x02)` 与 `PC_FEEDBACK_RETRY_REGION_2 (0x08)` 是 STM32 发给 PC 的一次性控制反馈，不在固定 50 Hz 批次列表中；固件内部有 pending 标志时追加到下一次发送批次，发送成功后自动清除。上位机应按 `cmd` 分发并把它们当作边沿事件处理，避免依赖持续电平。

`PC_FEEDBACK_START_MATCH` payload 为 1 字节，`start=1` 表示开始比赛或一区重试。该帧在操作手通过遥控器拨杆从已知非 PC 页切入 PC 页（`sw_l=UP && sw_r=UP`）的变化沿发送一次；MineInput 射频 `mode=1` 也复用该命令。上电初始状态为未知时不会因拨杆状态误发。

`PC_FEEDBACK_RETRY_REGION_2` payload 为 1 字节，固定发送 `retry=1`。该命令可由 MineInput 射频 `mode=2` 触发，也沿用原 RC 重试设计：PC 页下左摇杆 X 从未触发越过重试阈值时发送一次 `0x08`。固件不会周期发送该命令。
...
该反馈来自 UART8/UART7 上的 R1/R2 红外新协议帧：
...
所有合法命令都会刷新红外在线状态；`command` 不在 `1~4` 时按无效帧统计。RC02 成功解析并处理任一合法命令后，会从实际接收该命令的同一 UART 原路回发一次单字节 `0x5A`；每个合法帧回一次，重复合法帧也会分别回复，无效帧不回复。`0x5A` 只是接收确认数据，不带帧头，不应再作为命令送回 RC02。新一轮三层放矿流程为 `PC 发送 0x16 action=38 -> 红外 3 执行放矿循环 -> 红外 4 正常结束`。每次在机构就绪后收到的新命令 `3` 可触发一次放矿循环，循环完成后动作返回等待态，`busy` 仍保持 1，自动路由和完整资源租约均不释放。命令 `4` 使动作正常收尾；需要中止时 PC 发送 `0x16 action=1`。旧命令 `5/6` 无效。`0x9A` payload 为 3 字节，Python 解包格式为 `<BBB`。
...
| 3 | RELEASE；运行期间允许 `PC_CMD_CHASSIS (0x10)` 实时控制底盘，Pole/Arm/矿仓仍由 AutoOre 控制 |
...
UART8/UART7 简化帧为两个字节：`0xA5 command`。命令定义为 `1=在线`、`2=对接完成`、`3=允许放矿`、`4=结束并解除三层矿动作锁`，不使用 CRC。每次成功接收合法命令后，RC02 在同一 UART 回发一次单字节 `0x5A` 作为 ACK；`5/6` 为非法命令且不回复。
