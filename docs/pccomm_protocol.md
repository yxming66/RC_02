...
| `0x23` | `PC_CMD_AUTO_ACTION_V2` | 8 | `<HHBBBB` | 带事务标识的一键动作命令 |
...
- 新的非零 action 会立即接受。
- 同一个非零 action 连续高速发送不会重复触发；必须先发 `action=0` 重新布防，或改用带 `request_id` 的 `0x23`。
- `action=1` 为中止。
- `action=33` 为回收地面矿并存矿：机械臂到 0mm 取矿位吸盘开启，底盘低速前进，当前 SICK 有效且原始 ADC `<= auto_ore_param.recover_front_sick_adc_threshold`（默认 2800）时触发，再继续前进 50 ms，随后静止吸附、后退避障，抬矿确认后存入低/高矿仓空位；前 SICK 未在安全超时内触发时动作失败并停车。该阈值不再复用通用 `sick_front_ore_adc_min/max` 窗口。
...
其它 `state` 值按未准备完毕处理，即执行全灭灯效。取矛头等待对接动作会把 `state=1` 作为新协议的对接完成条件；UART8 红外命令 `02` 仍兼容。只有矿仓平台已到 `SPEARHEAD_DOCK_WAIT`、取矛头舵机当前姿态为 `DOCK_WAIT` 且到位后，固件才记录本机 ready 时间；对接完成帧的接收时间必须不早于该 ready 时间。提前发送的旧 `state=1`/`02` 不会在机构随后到位时补触发，发送端必须在机构就绪后重新发送。
...
payload 为 8 字节，Python pack 格式为 `<HHBBBB`：

| 偏移 | 类型 | 字段 | 说明 |
|---:|---|---|---|
| 0 | u16 | `request_id` | PC 生成的非零请求号；重发同一请求时保持不变 |
| 2 | u16 | `job_id` | `CONTINUE/ABORT/ACK` 时填写 STM32 返回的 Job ID；`START` 填 0 |
| 4 | u8 | `operation` | `1=START, 2=CONTINUE, 3=ABORT, 4=ACK` |
| 5 | u8 | `action` | `START` 时为 `PC_AutoAction_t`，其他操作可填 0 |
| 6 | u8 | `gate_id` | 当前 `1=RELEASE_STEP2`，非 CONTINUE 填 0 |
| 7 | u8 | `flags` | 保留，填 0 |

相同 `request_id + action` 的 START 是幂等重发，不会重新启动机构。新的请求在已有非终态 Job 时返回 `REJECTED/BUSY`。
...
| `0x9E` | `PC_FEEDBACK_AUTO_ACTION_V2` | 14 | `<HHBBBBBBHBB` | 一键动作 Job、分支和失败状态 |
...
2. `busy=0 && finished=1` 才表示动作状态机已经到达终态；底盘/台阶资源释放或分段完成不会提前设置 `finished=1`。
3. `RELEASE_STEP1`、`RELEASE_LIFT_DETECT_STEP1`、`ROD_SPEARHEAD_STEP1/STEP2`、`ROD_DOCK_WAIT` 上报真实结果；其 `result=1` 时必须读取 `failure_mask`。
4. 其他动作保留兼容策略：底层执行器到达 FAIL 后，对 PC 映射为 `result=0, failure_mask=0`。这是终态反馈映射，不会让动作提前结束。
...
8. 对单独取矿动作，PC 终态为成功时置 `bit0`；对单独存矿动作置 `bit1`；对普通上下台阶动作置 `bit2`。被兼容映射为成功的动作也补齐 required segment，使成功终态自洽。
...
payload 为 14 字节，Python unpack 格式为 `<HHBBBBBBHBB`。

| 偏移 | 类型 | 字段 | 说明 |
|---:|---|---|---|
| 0 | u16 | `request_id` | 对应 PC START 请求 |
| 2 | u16 | `job_id` | STM32 分配的非零 Job ID |
| 4 | u8 | `action` | Job 启动时的动作，续接阶段保持不变 |
| 5 | u8 | `state` | `0=IDLE,1=ACCEPTED,2=RUNNING,3=WAIT_GATE,4=ABORTING,5=SUCCEEDED,6=FAILED,7=ABORTED,8=REJECTED` |
| 6 | u8 | `required_mask` | Job 成功所需的分段位 |
| 7 | u8 | `running_mask` | 当前运行中的分段位 |
| 8 | u8 | `completed_mask` | 已成功分段位；同一 Job 内只增不减 |
| 9 | u8 | `failed_mask` | 已失败分段位；同一 Job 内只增不减 |
| 10 | u16 | `failure_mask` | 失败原因位，与 `0x96` 相同 |
| 12 | u8 | `reject_reason` | `0=NONE,1=INVALID_OP,2=INVALID_ACTION,3=BUSY,4=JOB_MISMATCH,5=NOT_WAIT_GATE,6=START_FAILED,7=QUEUE_FULL` |
| 13 | u8 | `active_node` | 当前底层动作 step/节点索引，仅用于观测 |

分段位定义与 `0x96` 相同。运行期间的完成位来自真实分支事件；当兼容策略把底层 FAIL 映射为 PC `SUCCEEDED` 时，终态会补齐 required segment，并清零 `failed_mask/failure_mask`。该映射只发生在底层执行器终止后。真实失败动作仍保留失败位；中止保持 `ABORTED`。终态结果保持到新的 START 覆盖或 PC 发送 `ACK(job_id)`。
...
| `1` | 在线 | 刷新红外在线状态 |
| `2` | 对接完成 | 刷新对接完成状态及时间戳 |
| `3` | 启动三层放矿并锁死 | 未锁定时建立本轮三层放矿锁；PC 观察到锁后用 V3 提交一次动作 38，重复命令 3 不产生新一轮 |
| `4` | 允许放矿 | 仅锁定期间刷新放矿许可；动作 38 本地准备完成后收到的新许可才有效 |
| `5` | 结束三层放矿锁死 | 解除本轮锁定、清除许可；动作 38 将 Arm 收回 `STANDBY`，到位后结束 |
| `6` | 不允许/中止放矿 | 撤销许可并让动作 38 回到 `WAIT_RELEASE_ORE` 保持；重复命令 6 不重复触发回撤 |

所有合法命令都会刷新红外在线状态；`command` 不在 `1~6` 时按无效帧统计。对接完成和允许放矿带 1000 ms 新鲜期。新一轮三层放矿必须按 `3 -> PC提交38 -> 4 -> 5`；需要中止时按 `3 -> PC提交38 -> 6 -> 5`。命令 6 优先执行回撤：Arm 回到 `WAIT_RELEASE_ORE` 后继续保持 Job、底盘锁和 Pole 资源，尚未释放矿时保持吸附，已经释放时保持吸盘关闭；此时不会提前产生终态。随后命令 5 才让 Arm 回到 `STANDBY`，到位后正常流程返回成功，中止流程返回 `CANCELLED`。payload 为 28 字节，Python unpack 格式仍为 `<12B4I`。
...
| 3 | u8 | `zone3_action_locked` | 命令 `3` 后为 1，命令 `5` 后为 0 |
| 4 | u8 | `release_allowed` | 命令 `4` 的放矿许可是否仍在新鲜期 |
| 5 | u8 | `release_abort_latched` | 是否已锁存命令 `6` 中止；命令 3/5 清除 |
| 6 | u8 | `zone3_action_state` | `0=未知, 1=已启动锁定, 2=已结束解锁` |
| 7 | u8 | `last_dock_complete_cmd` | 是否收到过命令 `2`，`0/1` |
| 8 | u8 | `last_zone3_action_cmd` | `0=无, 1=命令3启动, 2=命令5结束` |
| 9 | u8 | `last_release_cmd` | `0=等待, 1=命令4允许, 2=命令6中止` |
| 10 | u8 | `last_command` | 最近成功解析的原始命令，取值 `1~6` |
| 11 | u8 | `release_abort_count_lsb` | 有效命令 `6` 中止事件累计次数低 8 位 |
...
| 38 | RELEASE_IR_LIFT_DETECT，三层放矿单事务：锁死底盘、展开 Pole、上膛并到 `WAIT_RELEASE_ORE`，等待命令 4 后释放；释放或中止后回该等待位，命令 5 才回 `STANDBY` 并结束 |

命令 `3` 只建立红外锁，不再由 RC02 自动启动 legacy 动作。PC 检测到 `zone3_action_locked=1` 后，必须通过 V3 `SUBMIT(action=38)` 创建唯一 Job；动作 38 在整段事务中持续占有 CHASSIS、POLE、ARM、STORE 和共享阀资源，并持续输出底盘零速度。准备顺序为 Pole 展开、从矿仓上膛（如需要）、机械臂到 `WAIT_RELEASE_ORE`；平台、机械臂和 Pole 到位后才记录许可就绪时间。早于就绪时间的命令 `4` 不会补触发，R1 必须在就绪后重新发送。动作 38 完全忽略前方目标格光电/SICK 有无矿结果，没有 step1/step2；未收到有效命令 `4` 时无限保持在等待放矿位且不释放。收到 `4` 后执行完整放矿，完成后回到 `WAIT_RELEASE_ORE` 并继续保持 Job 和底盘锁。收到命令 `6` 也回到 `WAIT_RELEASE_ORE`，但记录本轮为中止且不提前结束 Job。只有命令 `5` 才让 Arm 回到 `STANDBY` 并在到位后产生终态：正常流程为 `SUCCEEDED`，发生过命令 6 的流程为 `CANCELLED`。

### 6.3 红外对接

UART8 简化帧为两个字节：`0xA5 command`。命令定义为 `1=在线`、`2=对接完成`、`3=启动三层放矿并锁死`、`4=允许放矿`、`5=结束三层放矿锁死`、`6=中止放矿`，不使用 CRC 或 ACK。
