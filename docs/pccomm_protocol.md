...
一键放矿流程还会直接发送灯效：目标为低层转换仓时发送 `0x04`，目标为高层转换仓时发送 `0x05`；三层抬升检测放矿进入等待被抬升阶段时发送 `0x02`；任意一键动作最终失败时发送 `0x03`。
...
| `0x96` | `PC_FEEDBACK_AUTO_ACTION` | 8 | `<BBBBHBB` | 一键动作简化结果 |
...
| `0x9C` | `PC_FEEDBACK_SICK_FRONT_ORE` | 16 | `<BBBBHHHHf` | 前 SICK 区域正方形矿检测 |
...
| 6 | u8 | `lower_finished` | 下层/底盘侧动作是否已成功完成，`0/1` |
| 7 | u8 | `upper_finished` | 上层机构侧动作是否已成功完成，`0/1` |
...
action, busy, finished, result, failure_mask, lower_finished, upper_finished = struct.unpack(
    "<BBBBHBB", payload
)
if busy:
    if lower_finished and not upper_finished:
        status = "LOWER_DONE_UPPER_RUNNING"
    else:
        status = "RUNNING"
...
payload 为 16 字节，Python unpack 格式为 `<BBBBHHHHf`。固件使用前光电 `rawdata[3]` 的 SICK 测距判断前方指定距离窗口内是否有矿；`detected=1` 表示样本持续落在窗口内并通过稳定时间确认。默认窗口由固件宏 `SICK_FRONT_ORE_DETECT_MIN_DISTANCE_MM`、`SICK_FRONT_ORE_DETECT_MAX_DISTANCE_MM` 和 `SICK_FRONT_ORE_DETECT_STABLE_MS` 配置。

| 偏移 | 类型 | 字段 | 说明 |
|---:|---|---|---|
| 0 | u8 | `sample_valid` | 前 SICK 样本有效，0/1 |
| 1 | u8 | `in_region` | 当前距离是否落在检测窗口内，0/1 |
| 2 | u8 | `detected` | 稳定确认后的矿检测结果，0/1 |
| 3 | u8 | `channel_index` | 使用的 SICK rawdata 通道，当前为 `3` |
| 4 | u16 | `adc_raw` | 前 SICK ADC 原始值 |
| 6 | u16 | `min_distance_mm` | 检测窗口近端，单位 mm |
| 8 | u16 | `max_distance_mm` | 检测窗口远端，单位 mm |
| 10 | u16 | `reserved` | 保留，当前固定为 `0` |
| 12 | f32 | `distance_mm` | 前 SICK 测距，单位 mm |
