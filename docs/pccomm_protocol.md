...
| `5` | 结束三层放矿锁死 | 解除本轮锁定、清除许可并通知动作 38 正常结束；重复命令 5 无副作用 |
| `6` | 中止放矿 | 产生一次安全中止事件并撤销许可；不依赖动作锁，重复命令 6 不重复触发回撤 |

所有合法命令都会刷新红外在线状态；`command` 不在 `1~6` 时按无效帧统计。对接完成和允许放矿带 1000 ms 新鲜期。新一轮三层放矿必须按 `3 -> PC提交38 -> 4 -> 5`；需要中止时按 `3 -> PC提交38 -> 6 -> 5`。命令 5 是正常结束事件：若尚未开始释放则直接结束，若释放已经开始则等待机械臂安全返回后结束。命令 6 是安全中止事件，优先于命令 5。payload 为 28 字节，Python unpack 格式仍为 `<12B4I`。
...
| 38 | RELEASE_IR_LIFT_DETECT，三层放矿单事务：锁死底盘、展开 Pole、上膛并到竖直位，等待命令 4 后释放，释放完成后继续持锁直到命令 5 |

命令 `3` 只建立红外锁，不再由 RC02 自动启动 legacy 动作。PC 检测到 `zone3_action_locked=1` 后，必须通过 V3 `SUBMIT(action=38)` 创建唯一 Job；动作 38 在整段事务中持续占有 CHASSIS、POLE、ARM、STORE 和共享阀资源，并持续输出底盘零速度。准备顺序为 Pole 展开、从矿仓上膛（如需要）、机械臂到竖直位；平台、机械臂和 Pole 到位后才记录许可就绪时间。早于就绪时间的命令 `4` 不会补触发，R1 必须在就绪后重新发送。动作 38 完全忽略前方目标格光电/SICK 有无矿结果，没有 step1/step2；未收到有效命令 `4` 时无限保持且不释放。收到 `4` 后执行完整放矿，完成后仍保持 Job 和底盘锁，直到命令 `5` 才进入成功终态。运行任意阶段收到命令 `6`，固件执行安全回撤并进入中止终态；PC 随后仍应等待/要求命令 `5` 解除红外锁。
