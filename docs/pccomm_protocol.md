...
- 同一个非零 action 连续发送不会重复触发；再次调用同一 action 前必须先发送 `action=0`，再发送目标 action。上位机也可对每次正常调用统一执行“发 0、发目标 action”。
...
| `0x96` | `PC_FEEDBACK_AUTO_ACTION` | 4 | `<BBBB` | 一键动作整体运行状态与终态结果 |
...
payload 固定为 4 字节，Python unpack 格式为 `<BBBB`。
...
| 38 | RELEASE_IR_LIFT_DETECT，红外协同三层放矿整体 action：`0x16 action=38` 启动，红外 `3` 执行放矿循环，红外 `4` 正常结束，`0x16 action=1` 中止 |
...
6. 调用 AutoAction 时发送 `0x16 action=0` 清 latch，再发送目标 action；等待相同 action 的 `0x96 busy=0 && finished=1` 后读取 `result`，才能调用下一动作或接管其资源。中止当前动作直接发送 `0x16 action=1`。
